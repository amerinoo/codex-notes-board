    ...
    <main className={`min-h-screen px-4 py-8 sm:px-6 ${isDark ? 'bg-stone-900 text-stone-100' : 'bg-stone-100 text-zinc-950'}`}>
      <div className="mx-auto max-w-6xl">
        <header className={`mb-8 flex flex-col gap-5 border-b pb-6 ${isDark ? 'border-stone-700' : 'border-stone-300'}`}>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <button
              className={`w-fit rounded-md border px-4 py-2 text-sm font-semibold transition ${
                isDark
                  ? 'border-stone-500 text-stone-100 hover:bg-stone-800'
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            {PRIORITY_OPTIONS.map((priority) => {
              ...
              return (
                <span
                  className={`rounded-md border px-2 py-1 font-semibold ${isDark ? priorityStyle.darkBadge : priorityStyle.badge}`}
                  key={priority}
                >
        ...
        <section className="grid gap-6 lg:grid-cols-[380px_1fr]">
          <form
            className={`rounded-lg border p-5 shadow-sm ${isDark ? 'border-stone-700 bg-stone-800' : 'border-stone-300 bg-stone-50'}`}
            onSubmit={saveNote}
          >
            <h2 className="text-xl font-semibold">{isEditing ? 'Edit note' : 'New note'}</h2>
            <label className={`mt-5 block text-sm font-medium ${isDark ? 'text-stone-200' : 'text-zinc-700'}`} htmlFor="title">
              Title
            </label>
            <input
              className={`mt-2 w-full rounded-md border px-3 py-2 text-base outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-200 ${
                isDark ? 'border-stone-600 bg-stone-900 text-stone-100 placeholder:text-stone-500' : 'border-stone-300 bg-white text-zinc-950'
              }`}
              id="title"
              name="title"
            ...
            <label className={`mt-4 block text-sm font-medium ${isDark ? 'text-stone-200' : 'text-zinc-700'}`} htmlFor="content">
              Content
            </label>
            <textarea
              className={`mt-2 min-h-40 w-full resize-y rounded-md border px-3 py-2 text-base outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-200 ${
                isDark ? 'border-stone-600 bg-stone-900 text-stone-100 placeholder:text-stone-500' : 'border-stone-300 bg-white text-zinc-950'
              }`}
              id="content"
              name="content"
            ...
            <label className={`mt-4 block text-sm font-medium ${isDark ? 'text-stone-200' : 'text-zinc-700'}`} htmlFor="priority">
              Priority
            </label>
            <select
              className={`mt-2 w-full rounded-md border px-3 py-2 text-base outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-200 ${
                isDark ? 'border-stone-600 bg-stone-900 text-stone-100' : 'border-stone-300 bg-white text-zinc-950'
              }`}
              id="priority"
              name="priority"
              onChange={updateField}
            ...
            {errorMessage && (
              <p className="mt-4 rounded-md border border-red-300 bg-red-100 px-3 py-2 text-sm text-red-800">{errorMessage}</p>
            )}
              ...
              {isEditing && (
                <button
                  className={`rounded-md border px-4 py-2 text-sm font-semibold transition ${
                    isDark ? 'border-stone-500 text-stone-100 hover:bg-stone-700' : 'border-stone-300 text-zinc-700 hover:bg-stone-200'
                  }`}
                  onClick={resetForm}
                  type="button"
                >
          ...
          <div>
            <label className={`block text-sm font-medium ${isDark ? 'text-stone-200' : 'text-zinc-700'}`} htmlFor="note-search">
              Search notes
            </label>
            <input
              className={`mt-2 w-full rounded-md border px-3 py-2 text-base outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-200 ${
                isDark ? 'border-stone-600 bg-stone-800 text-stone-100 placeholder:text-stone-500' : 'border-stone-300 bg-stone-50 text-zinc-950'
              }`}
              id="note-search"
            ...
            <div className="mt-4">
              {isLoading ? (
                <p className={`rounded-lg border p-5 ${isDark ? 'border-stone-700 bg-stone-800 text-stone-300' : 'border-stone-300 bg-stone-50 text-zinc-600'}`}>
                  Loading notes...
                </p>
              ) : notes.length === 0 ? (
                <p className={`rounded-lg border border-dashed p-8 text-center ${isDark ? 'border-stone-600 bg-stone-800 text-stone-300' : 'border-stone-300 bg-stone-50 text-zinc-600'}`}>
                    ...
                    return (
                      <li className={`rounded-lg border p-5 shadow-sm ${isDark ? priority.darkCard : priority.lightCard}`} key={note.id}>
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className={`shrink-0 rounded-md border px-2 py-1 text-xs font-semibold ${isDark ? priority.darkBadge : priority.badge}`}>
                            {priority.label}
                          </span>
                        </div>
                        <p className={`mt-3 whitespace-pre-wrap break-words text-sm leading-6 ${isDark ? 'text-stone-200' : 'text-zinc-700'}`}>
                        ...
                        <div className="mt-5 flex flex-wrap gap-2">
                          <button
                            className={`rounded-md border px-3 py-2 text-sm font-semibold transition ${
                              isDark
                                ? 'border-teal-300 text-teal-100 hover:bg-teal-950/60'
                          ...
                          <button
                            className={`rounded-md border px-3 py-2 text-sm font-semibold transition ${
                              isDark ? 'border-red-300 text-red-100 hover:bg-red-950/60' : 'border-red-300 text-red-700 hover:bg-red-50'
                            }`}
